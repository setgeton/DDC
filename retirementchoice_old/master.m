...
simdataseed = RandStream('mt19937ar','seed',311); 
...
clear;
gamm    = 2.0;      %scaling
thet   = 2.6;       %curvature of utility function
alph   = 1.3;    %cost of work 
...
t=0.3; def.t=t; %income tax
%t2=0.2; def.t2=t2;
maxpensexp = 45; %max years of work experience used for pension calculation
replrate= 0.6;  %replacement rate= share of last income
dedfactor=0.05; % actuarial deductions for every year of early retirement
def.lifeexp=80; % life expectancy

% generate monthly income for all individuals
mu_wage = 1.1; 
sd_wage = 0.3;
d.wage=NaN(N,NA);
d.wage = (exp(mu_wage + sd_wage*randn(N,1))*ones(1,NA));
...
d.workexp = round(30+15*rand(N,1))*ones(1,NA)+ones(N,1)*(0:NA-1); %work experience; uniform distribution on [30,45]
for period=1:NA
d.pensionb(:,period) = min([d.workexp(:,period)./maxpensexp,ones(N,1)],[],2) .*(1-dedfactor.*(NA-period)).*replrate.*d.wage(:,period) ;
end;
...
x0 = [0.5 2.5 0.5]; % starting values

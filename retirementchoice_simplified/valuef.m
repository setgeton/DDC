...
function[Vw,Vr] = valuef(data,pdef,param)
 ...
 Vr(:,maxA,firstbenefits) = ... 
   0.*  ( flowutility(param,benefits(:,maxA),retirement,W,t,evmeanN) ) ;
 Vr(:,maxA,benefitslastp) = ... 
   0.*  ( flowutility(param,benefits(:,maxA-1),retirement,W,t,evmeanN) ) ;
 ...
 Vr(:,recursiveage,firstbenefits) = ... 
     ...
     *(beta.^(0:agedeath-1-NRA+(maxA-recursiveage)))*ones(agedeath-1-NRA+(maxA-recursiveage)+1,1);
 if recursiveage>1
